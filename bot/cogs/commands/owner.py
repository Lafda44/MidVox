...
from core import Cog, midvox, Context
import sqlite3
import os
import requests
import numpy as np
...
class Owner(commands.Cog):
    ...
    async def staff_add(self, ctx, user: discord.User):
        if user.id in self.staff:
            sonu = discord.Embed(title=f"{ZWARNING}  Access Denied", description=f"{user} is already in the staff list.", color=0x8A9A5B)
            ...
            async with aiosqlite.connect(self.db_path) as db:
                ...
                await db.commit()
            sonu2 = discord.Embed(title=f"{TICK} Success", description=f"Added {user} to the staff list.", color=0x8A9A5B)
    ...
    async def staff_remove(self, ctx, user: discord.User):
        if user.id not in self.staff:
            sonu = discord.Embed(title=f"{ZWARNING} Access Denied", description=f"{user} is not in the staff list.", color=0x8A9A5B)
            await ctx.reply(embed=sonu, mention_author=False)
        else:
            ...
            async with aiosqlite.connect(self.db_path) as db:
                ...
                await db.commit()
                sonu2 = discord.Embed(title=f"{TICK} Success", description=f"Removed {user} from the staff list.", color=0x8A9A5B)
            await ctx.reply(embed=sonu2, mention_author=False)
    ...
    async def bdg(self, ctx):
        if ctx.invoked_subcommand is None:
            embed = discord.Embed(description='Invalid `bdg` command passed. Use `add` or `remove`.', color=0x8A9A5B)
            await ctx.send(embed=embed)
    ...
    async def add(self, ctx, member: discord.Member, badge: str):
        ...
        if badge in BADGE_URLS or badge == 'bug' or badge == 'all':
            if badge == 'all':
                ...
                embed = discord.Embed(description=f"All badges added to {member.mention}.", color=0x8A9A5B)
                await ctx.send(embed=embed)
            else:
                success = add_badge(user_id, badge)
                if success:
                    embed = discord.Embed(description=f"Badge `{badge}` added to {member.mention}.", color=0x8A9A5B)
                else:
                    embed = discord.Embed(description=f"{member.mention} already has the badge `{badge}`.", color=0x8A9A5B)
                await ctx.send(embed=embed)
        else:
            embed = discord.Embed(description=f"Invalid badge: `{badge}`", color=0x8A9A5B)
            await ctx.send(embed=embed)
    ...
    async def remove(self, ctx, member: discord.Member, badge: str):
        ...
        if badge in BADGE_URLS or badge == 'bug' or badge == 'all':
            if badge == 'all':
                ...
                embed = discord.Embed(description=f"All badges removed from {member.mention}.", color=0x8A9A5B)
                await ctx.send(embed=embed)
            else:
                success = remove_badge(user_id, badge)
                if success:
                    embed = discord.Embed(description=f"Badge `{badge}` removed from {member.mention}.", color=0x8A9A5B)
                else:
                    embed = discord.Embed(description=f"{member.mention} does not have the badge `{badge}`.", color=0x8A9A5B)
                await ctx.send(embed=embed)
        else:
            embed = discord.Embed(description=f"Invalid badge: `{badge}`", color=0x8A9A5B)
            await ctx.send(embed=embed)
...
class Badges(commands.Cog):
    ...
    async def profile(self, ctx: commands.Context, member: discord.Member = None):
        ...
        embed = discord.Embed(color=0x8A9A5B)
