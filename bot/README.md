...
<h3>MidVox Bot — Python Discord Bot + FastAPI Backend</h3>
...
This folder contains the MidVox Discord bot built on `discord.py v2` alongside a `FastAPI` backend that powers the web dashboard. Everything runs from a single `python CodeX.py` command.
...
│   └── midvox/             Core MidVox feature cogs
...
2. Choose **Cloudflared**, name it (e.g. `midvox-api`), save
