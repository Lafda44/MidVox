...
<h3>MidVox Dashboard — Next.js Web Interface</h3>
...
This folder contains the MidVox web dashboard built with `Next.js 14` (App Router), `TypeScript`, and `Tailwind CSS`. It connects to the bot's FastAPI backend via a permanent Cloudflare Tunnel HTTPS URL and lets server admins manage all bot settings through a sleek, branded UI.
...
|---|---|
| Node.js 18+ | — |
| MidVox bot running | with `API_ENABLED=true` and `TUNNEL_ENABLED=true` |
